document.addEventListener("DOMContentLoaded", () => {
  ...
  function clearOut() {
    out.querySelectorAll("div").forEach((div) => div.remove());
  }

  function makeEditor(id, mode, ace) {
  ...
  const ed_HTML = makeEditor("ed_html", "ace/mode/html", ace);
  const ed_css = makeEditor("ed_css", "ace/mode/css", ace);
  const ed_js = makeEditor("ed_js", "ace/mode/javascript", ace);
  ...
  showPane("html");
  function buildSrcdoc(withTests = false) {
  ...
  function loadProject(obj) {
    try {
      ...
      if (obj.kind === "web-only") ed_HTML.setValue(obj.html || "", -1);
